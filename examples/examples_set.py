...
def run_swe_simulation(
    ...
    if solver.rank == 0 and solver.config.output_dir is not None:
        tidalflow.utils.visualization.animate_solution(
            ...
            dark_mode=dark_mode,
            writer=writer,
            file_name=f"{name}_velocity.{'mp4' if writer == 'ffmpeg' else 'gif'}",
            fps=fps,
        ...
        tidalflow.utils.visualization.animate_surface(
            ...
            dark_mode=dark_mode,
            writer=writer,
            file_name=f"{name}_surface.{'mp4' if writer == 'ffmpeg' else 'gif'}",
            fps=fps,
...
def example_fort_myers() -> None:
    """Example: SWE simulation for Fort Myers, FL."""

    fort_myers_location = (26.647778, -81.87111)
    offset = 0.1

    run_swe_simulation(location=fort_myers_location, offset=offset, name="fort_myers")
...
def example_port_canaveral() -> None:
    """Example: SWE simulation for Port Canaveral, FL."""

    port_canaveral_location = (28.4158, -80.5931)
    offset = 0.1

    run_swe_simulation(
        location=port_canaveral_location, offset=offset, name="port_canaveral"
    )
